...
#if defined(NDEBUG)
# error "Aonecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5a7e1c679ef5f2dbad3c2f6f859ae277f3aeeaf0190c9ed4ea24a6989cca62f6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aonecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aonecoin Signed Message:\n";
...
{
    // Aonecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Aonecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // Aonecoin: 4 days
static const int64 nTargetSpacing = 4 * 60; // Aonecoin: 4 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aonecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aonecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5a7e1c679ef5f2dbad3c2f6f859ae277f3aeeaf0190c9ed4ea24a6989cca62f6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Aonecoin, The Next Generation Anonymous Cryptocurrency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xa55a2979f57288318a2c24559226154a7ac289b19c28dde7a56d34a73f35bf81"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Aonecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AonecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AonecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AonecoinMiner(CWallet *pwallet)
{
    printf("AonecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aonecoin-miner");
    ...
    try { loop {
        ...
        printf("Running AonecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AonecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AonecoinMiner, pwallet));
}
